...
export const TaskList = () => {
  ...
  return (
    <div className="flex flex-col gap-2 bg-gray-900 rounded-md p-5 ">
      ...
      <h1 className="text-center text-xl font-medium" > Task List </h1>
      ...
      {tasks.map(task => (
          ...
          >
            <div className={`flex flex-col items-start justify-between`} >
              <h5 className={ `font-medium text-xl  border-r-indigo-600  |  ${task.done ? "line-through" : ""}` }
              >{task.title}</h5>

              <p className={ `font-light  |  ${task.done ? "line-through" : ""}`}
              >{task.description}</p>
            </div>
